...
info_data = []
info_header = []
#info_header = ["name","full_name","team","place_of_birth","eye_color","hair_color","base"]

with open("sh_info.csv") as file_object:
#with open(csv_name) as file_object:
	csv_content = csv.reader(file_object, delimiter = ",")
	row = 0
	for i in csv_content:
		if row == 0:
			info_header.extend(i)
			row += 1
		else:
			row += 0
			info_data.append(i)
...
json_name = "sh_powers.json"

with open(json_name) as file_object:
	powers = json.load(file_object)
...
def get_info(row, header):
	superhero_info = dict(zip(header,row))
...
for line in info_data:
	unique_hero_info = get_info(line, info_header)
	heroes[unique_hero_info['name']] = unique_hero_info
# Add the list of powers with another for-loop:
for key,values in powers.items():
	for line in heroes:
		if key == heroes[line]['name']:
			heroes[line]['powers'] = values
# Finally write out the whole dictionary as a .json file:
with open ('heroes.json','w') as file_object:
	json.dump(heroes, file_object)
...
for key,value in heroes.items():
	if 'new york city' in value['base'].lower() and 'super_strength' in value['powers']:
